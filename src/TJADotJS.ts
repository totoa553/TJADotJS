...
export default class {
  ...
  */
  constructor(str:string){
    ...
    var splitedCourses = str.replaceAll(/\/\/(\S+)/mg,"").split("COURSE:");
    ...
    for(var i = 0; i < courses.length; i++){
      function setComposite(course: string,first: boolean){
        var [header, text, remain, side] = parentthis.SplitCourse(course);
        console.log([header, text, remain, side])
        ...
        function getMeasureThisPlayerSide(playerMeasure: string){
          ...
          for(var nowLine of playerMeasure.split("\n")){
        ...
        this.chart.Courses.forEach (function(course: Course){
          function parseTJA(list: Chip[],measures: string[])
          {
              ...
              measures.forEach (function(measure){
                ...
                measure.split("\n").filter(a=>!(a=="")).forEach(function(line){
                    if (!line.startsWith("#"))
                    {
  ...
  SplitCourse(str: string){
    ...
    for (var lines = 0; lines < split.length; lines++)
    {
      ...
      {
        splitHeader = split.slice(0,lines).join("\n");
        // #ENDまでTakeしたい
